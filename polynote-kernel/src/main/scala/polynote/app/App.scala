...
trait App extends Runtime[Clock with System with Blocking with Logging] {
  ...
  override val environment: Environment = new Clock.Live with Console.Live with system.System.Live with Random.Live with Blocking.Live with Logging.Live

  protected def reportFailure(cause: Cause[Any]): Unit =
  ...
  override val platform: Platform = new Platform {
    ...
    def reportFailure(cause: Cause[Any]): Unit = App.this.reportFailure(cause)
