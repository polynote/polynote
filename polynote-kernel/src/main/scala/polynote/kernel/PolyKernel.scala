...
class PolyKernel private[kernel] (
  ...
  private val kernels = new ConcurrentHashMap[String, LanguageKernel[IO, GlobalInfo]]()
  ...
  protected def getKernel(language: String): IO[LanguageKernel[IO, GlobalInfo]] = Option(kernels.get(language)).map(IO.pure).getOrElse {
    launchingKernel.acquire.bracket { _ =>
      Option(kernels.get(language)).map(IO.pure).getOrElse {
        for {
          ...
          kernel  <- taskQueue.runTask(s"Kernel$$$language", s"Starting $language kernel")(_ => factory.apply(dependencies.getOrElse(language, Nil), globalInfo))
  ...
  protected def withKernel[A](cellId: String)(fn: (Notebook, NotebookCell, LanguageKernel[IO, GlobalInfo]) => A): IO[A] = for {
  ...
  private def parentContext(notebook: Notebook, id: String) = Option(runtimeContextHolder.get(parentCellId(notebook, id))).getOrElse(throw new Exception("Well, how did this happen..."))

  private val runtimeContextHolder: ConcurrentHashMap[String, RuntimeContext[GlobalInfo]] = new ConcurrentHashMap[String, RuntimeContext[GlobalInfo]] {
    val predef = RuntimeContext.getPredefContext[globalInfo.type](globalInfo)
      ...
      oq =>
          ...
          oq.enqueue1(Some(ClearResults())) *> withKernel(id) {
            (notebook, cell, kernel) =>
              val cellContext = parentContext(notebook, id)
                ...
                taskInfo =>
                  ...
                  kernel.runCode(
                    ...
                    case (results, newCtx) =>
                      ...
                      newCtx.map { ctx =>
                        runtimeContextHolder.put(id, ctx)
                      }
                  }.handleErrorWith {
  ...
  def completionsAt(id: String, pos: Int): IO[List[Completion]] = withKernel(id) {
    (notebook, cell, kernel) =>
      kernel.completionsAt(id, parentContext(notebook, id), cell.content.toString, pos)
  }.flatMap(identity)

  def parametersAt(id: String, pos: Int): IO[Option[Signatures]] = withKernel(id) {
    (notebook, cell, kernel) =>
      kernel.parametersAt(id, parentContext(notebook, id), cell.content.toString, pos)
  }.flatMap(identity)
