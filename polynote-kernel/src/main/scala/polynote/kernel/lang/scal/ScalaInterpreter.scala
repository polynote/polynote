        ...
        case sym =>
          ...
          val run = IO(importToRuntime.importSymbol(sym)).flatMap {
            runtimeSym =>
              ...
              (symbolTable.publishAll(syms), syms.map(rv => out.enqueue1(Output("text/scala; rel=decl", s"${rv.name.toString}: ${rv.typeString} = ${rv.valueString}"))).sequence).parMapN((_, _) => ())
          }
          ...
          val eval = Queue.unbounded[IO, Option[Chunk[Byte]]].map(new QueueOutputStream(_)).bracket {
            stdOut =>
              val results = stdOut.queue.dequeue
                ...
                .map(Output("text/scala; rel=stdout", _))
