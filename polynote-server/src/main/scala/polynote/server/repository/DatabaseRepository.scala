...
import polynote.server.repository.fs.DatabaseFilesystem
...
class DatabaseRepository(
  val path: Path,
  val fs: DatabaseFilesystem,
  val chunkSize: Int = 8192,
  val defaultExtension: String = "ipynb"
) extends NotebookRepository {
  protected def pathOf(relativePath: String): Path = Paths.get(relativePath)

  private final class DbNotebookRef private (
  ...
  private object DbNotebookRef {
    ...
    def apply(notebook: Notebook, version: Int): RIO[BaseEnv with GlobalEnv, DbNotebookRef] = for {
      log        <- Logging.access
      ...
      ref         = new DbNotebookRef(current, pending, closed, log, renameLock, process, wal)
  ...
  override def loadNotebook(path: String): RIO[BaseEnv with GlobalEnv, Notebook] = for {
    ...
    content        <- fs.readPathAsString(pathOf(extractExtension(path)._1))
  ...
  override def createAndOpen(path: String, notebook: Notebook, version: Int): RIO[BaseEnv with GlobalEnv, NotebookRef] = for {
    ...
    ref <- DbNotebookRef(notebook, version)
  } yield ref
